...
namespace LogMaintenance
{
    public static class BlobHelper
    {
        ...
        public static IEnumerable<BlobFileInfo> ListFolderBlobs(string containerName)
        {
            ...
            foreach (IListBlobItem item in container.ListBlobs(null, false))
            {
                if (item.GetType() == typeof(CloudBlockBlob))
                {
                    ...
                    var blobFilePath = blob.Uri.AbsolutePath.Replace(blob.Container.Uri.AbsolutePath + "/", "")
                        .Replace("%20", " ");
                ...
                else if (item.GetType() == typeof(CloudBlobDirectory))
                {
                    ...
                    CloudBlobContainer container2ndLevel =
                        blobClient.GetContainerReference(item.Uri.ToString().Replace(blobClient.BaseUri.ToString(), "")
                            .TrimEnd('/'));
        ...
        public static object FindLogFile(string containerName, string path)
        {
            ...
            var blobDirectory = blobContainer.GetDirectoryReference(path);
