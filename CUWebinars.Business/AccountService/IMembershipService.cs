...
namespace CUWebinars.Business.AccountService
{
    public interface IMembershipService : IDisposable
    {
        ...
        void SignIn(UserAccount userAccount, bool persistent);
        void UpdateNameTitle(
            ...
            string email,
            string title);
        void UpdateUserDetails(string tenant, string firstName, string lastName, string email, string institutionName, Address billingAddress, Address shippingAddress, string title, int? sageAccountId, int? timeZone);
        ...
        void AddClaimForPostEventMaterials(string email, OrderRow row, DateTime expiryDate, string tenant);
