...
SELECT  [Key] ,
        ...
        VerificationStorage
FROM    UserAccounts
WHERE   IsAccountVerified = 0
        OR VerificationKey IS NOT NULL
        OR VerificationPurpose IS NOT NULL
        OR VerificationKeySent IS NOT NULL
        OR VerificationStorage IS NOT NULL
...
SELECT  *
FROM    UserAccounts
WHERE   LastFailedLogin IS NOT NULL
        OR FailedLoginCount > 0
        OR IsAccountClosed > 0
...
/************************************************************************************************************/
SELECT  ua.Email ,
        ua.[Key] -- add any more cols desired
FROM    UserAccounts ua
WHERE   NOT EXISTS ( SELECT *
                     FROM   WebUser wu
                     WHERE  wu.email = ua.Email )
...
SELECT  wu.email ,
        wu.idUser -- add any more cols desired
FROM    WebUser wu
WHERE   email LIKE '%@notauthenticated.com'
