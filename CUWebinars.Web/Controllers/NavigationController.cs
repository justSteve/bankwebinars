...
namespace CUWebinars.Web.Controllers
{
    public class NavigationController : Controller
    {
        ...
        public string GetCcsModules()
        {
            var desWebinars = _webinarRepository.GetCcs().Where(w => w.idWebinar != 2485).OrderByDescending(w => w.Date).Take(8).ToList();

            var recordedWebinarsListItems = new StringBuilder();

            recordedWebinarsListItems.Append(
                "<li role='presentation'><a  role=\"menuitem\" tabindex=\"-1\"  href='/Webinar/allActive/?eventsToShow=CCS'>View <b>All</b> Courses</a></li>"
                );
