...
namespace CUWebinars.Web.Controllers.Admin
{
    ...
    public class AdminController : Controller
    {
        ...
        {
            if (model.TemplateType == "OCL")
            {
                model.Webinar = _webinarManagementService.GetWebinar(model.Webinar.idWebinar);
                model.SubscriptionPackURL = _globalConfig.TenantURL + "/WSP";
                ...
                model.EventBody =
                    HttpUtility.HtmlDecode(
                        _generalFormatter.FormatV2(model, "~/Notification/Templates/SendOCLPromo.cshtml").Body);
                // get Template with new method

                return Json(new { masterText = model.EventBody });
            }

            if (model.TemplateType == "ACS")
            {
                ...
                model.EventBody =
                    HttpUtility.HtmlDecode(
                        _generalFormatter.FormatV2(model, "~/Notification/Templates/SendACSPromo.cshtml").Body);
                // get Template with new method
            ...
            if (model.TemplateType == "DES")
            {
                ...
                // begin instrumentation of MC stats
                //model.ShowMcCampaignViewModel = new ShowMcCampaignViewModel
                //{
                //    Affiliates = _affiliateManagementService.GetAffiliates()
                //        .Where(a => a.idMailChimpList != ""),
                //    McCampaigns = new IEnumerable<McCampaign>,
                //    Webinar = model.Webinar
                //};

                //foreach (var affiliate in model.ShowMcCampaignViewModel.Affiliates)
                //{
                //    var affCamp = _webinarManagementService.GetCampaignsByAffiliate(affiliate.idUserAff,
                //        model.ShowMcCampaignViewModel.Webinar.idWebinar);
                //    model.ShowMcCampaignViewModel.McCampaigns.Add();
                //}
        ...
        [HttpGet]
        public async Task<ActionResult> ReminderPromo(int id)
        {

            TempData["campaigns"] = _webinarManagementService.GetWebinar(id).Campaigns;
            ...
            if (claimsIdentityOfAuthenticatedUser.HasClaim((claim) => claim.Type == Business.Constants.ClaimTypes.Admin))
            {
                //hack GetAffiliatesByPromoType("Daily") is hacked to just always list all affiliates. Is nothing to do with 'Daily' template.
                model.Affiliates = new AffiliateRepository().GetAffiliatesByPromoType("Daily").ToList();
            }
            else
            {
        ...
        [HttpGet]
        public async Task<ActionResult> PromoForACS(int id)
        {
            ...
            {
                //hack GetAffiliatesByPromoType("Daily") is hacked to just always list all affiliates. Is nothing to do with 'Daily' template.
                model.Affiliates = new AffiliateRepository().GetAffiliatesByPromoType("ACS").ToList();
            }
            else
            {
                return null;
        ...
        [HttpGet]
        public async Task<ActionResult> PromoForOCL(int id)
        {
            ...
            {
                //hack GetAffiliatesByPromoType("Daily") is hacked to just always list all affiliates. Is nothing to do with 'Daily' template.
                model.Affiliates = new AffiliateRepository().GetAffiliatesByPromoType("OCL").ToList();
            }
            else
            {
                return null;
        ...
        public async Task BuildMailChimpCampaignRecords()
        {
            try
            {

                var originalCampaign = JsonConvert.DeserializeObject<List<CampaignsJson>>
                ...
                foreach (var id in originalCampaign)
                {
                    ...
                    try
                    {
                        ...
                        {
                            _logger.Warn("BuildMailChimpCampaignRecords didn't find title: " + campaign.Settings.Title + " CampaignId: " + campaign.Id);
                            continue;
                        };
                        ...
                        var affId = _affiliateManagementService.LoadByTTSDomain(affString).idUserAff;

                        if (affId == null)
                        {
                            _logger.Warn("BuildMailChimpCampaignRecords didn't find affiliate: " + affString +
                        ...
                        var usersFromSheet =
                            dataOperations.InsertMailChimpCampaign(campaign);

                    }
                    catch (Exception ex)
                    {
                        _logger.FatalException("BuildMailChimpCampaignRecords: " + id, ex);
                    }
                }
            }
            catch (Exception exception)
        ...
        [HttpPost]
        [HandleAjaxException]
        public async Task<JsonResult> GenerateMailChimpCampaign
            (int affiliateId, string messageBodyHtml, int webinarId,
            string sendDate, string sendTime, string subject, string isReminder)
        {
            var acsLink = "";
            if (subject.Contains(" | "))
            {
                acsLink = subject.Split('|')[1].Trim();
            ...
            if (messageBodyHtml.Contains("{acsLink}"))
            {
                messageBodyHtml = messageBodyHtml.Replace("{acsLink}", acsLink);
            }
            if (flag)
            {
            ...
            try
            {
                CUWebinars.Business.ModelsV4.McCampaign campaign = new CUWebinars.Business.ModelsV4.McCampaign
                { AffiliateId = affiliate.idUserAff, WebinarId = webinar.idWebinar };
                ...
                var reminderAddresses = "";
                var segments = await manager.ListSegments.GetAllAsync(affiliate.idMailChimpList).ConfigureAwait(false);
                if (segments == null)
                {
                    _logger.Warn("segment not found " + affiliate.ttsDomain);
                    return Json(new
                    {
                        Result = "segment not found " + affiliate.ttsDomain + Environment.NewLine + Environment.NewLine
                    });
                }

                var savedSegId = segments.FirstOrDefault(s => s.Name.ToLower() == "webinars").Id;
                ...
                if (isReminder == "true")
                {
                    try
                    {
                        subject = subject.Replace("Webinar", "Final Reminder");

                        var idCampaign =
                            _webinarManagementService.GetCampaignsByAffiliate(affiliateId, webinar.idWebinar);
                        if (idCampaign == "not found")
                        {
                            _logger.Warn(
                                "idCampaign not found " + affiliate.ttsDomain);

                            return Json(new
                            { Result = "idCampaign not found " + affiliate.ttsDomain + Environment.NewLine });

                        }

                        if (idCampaign == null || idCampaign == "")
                        {
                            _logger.Warn(
                                "idCampaign is null " + affiliate.ttsDomain);

                            return Json(
                                new { Result = "idCampaign is Null " + affiliate.ttsDomain + Environment.NewLine });
                        }

                        campaign.CampaignId = idCampaign;
                        var dataOperations =
                            new DataOperations(ConfigurationManager.ConnectionStrings["DefaultConnection"]
                                .ConnectionString);

                        var cmpRpt = await manager.Reports.GetClickReportAsync(idCampaign);
                        foreach (var urlClicked in cmpRpt.Where(c => c.TotalClicks > 0))
                        {
                            var clickDetail = await manager.Reports
                                .GetClickReportDetailsAsync(idCampaign, urlClicked.Id).ConfigureAwait(false);

                            if (clickDetail == null)
                            {
                                _logger.Warn(
                                    "ClickDetail is null " + affiliate.ttsDomain);
                                return Json(new { Result = "reportDetails not found " + affiliate.ttsDomain });
                                ;
                            }

                            var clicked =
                                dataOperations.InsertMailChimpUrlClicked(urlClicked);

                            var members = await manager.Reports.GetClickReportMembersAsync(idCampaign, clickDetail.Id);

                            if (members == null || members.Count() == 0)
                            {
                                _logger.Warn(
                                    "members list is empty " + affiliate.ttsDomain);
                                return Json(new { Result = "Members list is empty " + affiliate.ttsDomain });
                                ;
                            }

                            try
                            {
                                foreach (var clickMember in members)
                                {
                                    var userHasOrder =
                                        _orderManagementService.CheckIfEmailAlreadyRegisteredForWebinar(webinarId,
                                            clickMember.EmailAddress);
                                    if (userHasOrder == 0)
                                    {
                                        if (clickDetail.Url.Contains("/details/" + webinarId))
                                        {
                                            _logger.Info(
                                                "GMCC: Send Reminder to: " + clickMember.EmailAddress + " UrlId: " +
                                                clickMember.UrlId + " on webinar: " + webinar.idWebinar);
                                            auditBuilder.Append(
                                                "Send Reminder to: " + clickMember.EmailAddress + " UrlId: " +
                                                clickMember.UrlId + " on webinar: " + webinar.idWebinar +
                                                Environment.NewLine);
                                            reminderAddresses += "," + clickMember.EmailAddress;
                                        }
                                        else
                                        {
                                            _logger.Info(
                                                "GMCC: Send Reminder found clicked link that was not primary target: " +
                                                clickMember.EmailAddress + " Link: " + clickDetail.Url);
                                            //errorsBuilder.Append("Send Reminder found clicked link that was not primary target: " + clickMember.EmailAddress + " Link: " + clickDetail.Url + Environment.NewLine);
                                        }
                                    }
                                    else
                                    {
                                        _logger.Info(
                                            "GMCC: Send Reminder will not send to: " + clickMember.EmailAddress);
                                        auditBuilder.Append("Send Reminder will not send to: " +
                                                            clickMember.EmailAddress);
                                    }
                                }
                            }
                            ...
                            {
                                _logger.FatalException("GMCC MemberLoop:  " + affiliate.ttsDomain, e);
                                return Json(new { Result = "GMCC MemberLoop: " + e.Message + Environment.NewLine });
                            }
                        }

                        if (reminderAddresses == "")
                        {
                            _logger.Info(
                                "GMCC: blank list of addresses:  " + affiliate.ttsDomain);

                            return Json(new
                            {
                                Result = "blank list of addresses:  " + affiliate.ttsDomain + Environment.NewLine +
                                         Environment.NewLine
                            });
                        }
                        ...
                        {
                            Name = "Reminder_" + campaign.CampaignId + "_aff_" + campaign.AffiliateId,
                            EmailAddresses = reminderAddresses.ToString().TrimStart(',').Split(',')
                        };

                        var newSeg = await manager.ListSegments.AddAsync(list.Id, mySeg).ConfigureAwait(false);
                        if (newSeg == null)
                        {
                            _logger.Info(
                                "GMCC NewSeg is null:   " + affiliate.ttsDomain);
                            return Json(new
                            {
                                Result = "New segment is null: :  " + affiliate.ttsDomain + Environment.NewLine +
                                         Environment.NewLine
                            });

                        }

                        savedSegId = newSeg.Id;
                    }
                    catch (Exception e)
                    {
                        _logger.FatalException("GMCC:  " + affiliate.ttsDomain, e);
                        return Json(new { Result = "GMCC: " + e.Message });
                    }
                }
                ...
                try
                {
                    if (_globalConfig.Tenant == "CCS")
                        savedSegId = segments.FirstOrDefault(s => s.Name.ToLower() == "ccs").Id;

                    if (_globalConfig.Tenant == "DirectorSeries")
                        savedSegId = segments.FirstOrDefault(s => s.Name.ToLower() == "des").Id;

                    recp = new Recipient
                    {
                        ListId = affiliate.idMailChimpList,
                        SegmentOptions =
                            new SegmentOptions { SavedSegmentId = savedSegId }
                    };
                }
                catch (Exception e)
                {
                    _logger.FatalException("No Segment Found for:  " + affiliate.ttsDomain, e);
                    return Json(new
                    {
                        WebUiConstants.Fail,
                        campMsg = "No Segment Found for:  " + affiliate.ttsDomain + Environment.NewLine +
                                  Environment.NewLine
                    });
                }
                ...
                if (_globalConfig.Tenant == "CCS")
                    newCamp.Settings.FolderId = "cf6d051d78";

                if (_globalConfig.Tenant == "CUWebinars")
                    newCamp.Settings.FolderId = "485a0396c0";

                if (_globalConfig.Tenant == "DirectorSeries")
                    newCamp.Settings.FolderId = "23bf826cc3";

                if (_globalConfig.Tenant == "MortgageWebinars")
                    newCamp.Settings.FolderId = "04f95c484f";

                if (isReminder == "true")
                {
                    newCamp.Settings.FolderId = "a640c13260";
                }
                ...
                var putContent =
                    await manager.Content.AddOrUpdateAsync(mkCamp.Id, new ContentRequest { Html = messageBodyHtml });
                if (isReminder != "true")
                {
                    campaign.Type = "Promo";
                    ...
                    _logger.Info(
                        "GenerateMailChimpCampaign | sendDate: " + sendDate);
                    auditBuilder.Append(" sendDate: " + sendDate + " id: " + mkCamp.Id + " with emails: " +
                                        reminderAddresses.ToString() + Environment.NewLine);
                ...
                {
                    _logger.Info(
                        "GenerateMailChimpCampaign | sendDate: " + sendDate + " with emails: " +
                        reminderAddresses.ToString());
                    auditBuilder.Append(
                        "SendDate: " + sendDate + " with emails: " + reminderAddresses.ToString() +
                        Environment.NewLine);
                ...
                return Json(new
                {
                    WebUiConstants.Success,
                    campaign,
                    audit = auditBuilder.ToString(),
        ...
        public async Task<ActionResult> MailChimpUpdateAffiliateLists()
        {
            ...
            foreach (var affiliate in affiliates)
            {
                ...
                try
                {
                    ...
                    var writeWhy = await manager.Lists.AddOrUpdateAsync(mcList).ConfigureAwait(false);
                    ...
                    {
                        await manager.ListSegments.DeleteAsync(mcList.Id, tSegment.Id.ToString()).ConfigureAwait(false);
                    }
                    ...
                    foreach (var group in groups)
                    {
                        try
                        {
                            ...
                            foreach (var interest in interests)
                            {
                                ...
                                Condition sc = new Condition
                                {
                                    ...
                                    Field = "interests-" + group.Id,
                                    Value = JArray.FromObject(new string[] { interest.Id })
                                };
                                ...
                                var mySeg = new Segment
                                {
                                    Options = so,
                                    Name = interestTitle,
                                    //EmailAddresses = members.Select(m => m.EmailAddress).ToArray()
                                };
                                ...
                                if (!itExists.Any())
                                {
                                    ...
                                    var newSeg = await manager.ListSegments.AddAsync(mcList.Id, mySeg).ConfigureAwait(false);
                                    //string[] addMem = members.Select(m => m.EmailAddress).ToArray();


                                    //var addMembers =
                                    //    await manager.ListSegments.BatchMemberAsync(mcList.Id
                                    //    , newSeg.ListId, new BatchSegmentMembers
                                    //    {
                                    //        MembersToAdd = addMem
                                    //    });
                                    ...
                                    //    {
                                    //        Method = "PUT",
                                    //        Path = $"/lists/{mcList.Id}/members/{manager.Members.Hash(x.EmailAddress.ToLower())}",
                        ...
                        {
                            _logger.FatalException("MailChimpUpdateAffiliateLists interest groups: ", e);
                        }
                    }
                    try
                    {
                    ...
                    {
                        _logger.FatalException("MailChimpUpdateAffiliateLists inner: ", e);
                    }
                }
                catch (Exception exception)
                {
                    _logger.FatalException("MailChimpUpdateAffiliateLists: ", exception);
                    //return Json(new { Result = exception.Message });
                }
            }
        ...
        public async Task<ActionResult> MailChimpMigrateUsers(string aff)
        {
            ...
            foreach (var affiliate in affiliates.OrderBy(a => a.ttsDomain))
            {
                ...
                try
                {
                    ...
                    var intWebinars = interests.SingleOrDefault(i => i.Name.Contains("ebinars.com")).Id;

                    var totalCount = usersFromSheetForAff.Count();
                    sb.Append("Total users to be imported: " + totalCount + "<br>");
                    var iterator = 0;
                    if (totalCount == 0)
                        continue;
                    ...
                    {
                        iterator++;
                        Console.WriteLine("processing: " + user.Email_Address + iterator + " of " + totalCount);
                        ...
                        {
                            _logger.FatalException("MigrateMailChimpUser: ", e);
                        }
                        var text = "";
        ...
        [HttpGet]
        [HandleAjaxException]
        public async Task<ActionResult> MailChimpUpdateLinkId(string aff)
        {
            _logger.Info("MailChimpUpdateLinkId starting");
            var affiliates = _affiliateManagementService.GetAffiliates();
            var model = new ListSegmentsViewModel
            {
                CFT = new List<Affiliate>(),
            ...
            foreach (var affiliate in affiliates)
            {
                ...
                sb.Clear();
                sb.Append("MailChimpUpdateLinkId starting: " + affiliate.ttsDomain.ToLower() + "<br>");

                var mcList = lists.FirstOrDefault(l => l.Id == affiliate.idMailChimpList);
                if (mcList == null)
                {
                    _logger.Warn("No List found for " + affiliate.ttsDomain + "<br>");
                    sb.Append("No List found for " + affiliate.ttsDomain + "<br>");
                ...
                try
                {
                    members = await manager.Members.GetAllAsync(mcList.Id).ConfigureAwait(false);
                    ...
                    foreach (var user in members)
                    {
                        ...
                        //}
                        var linkVar = "";
                        if (user.MergeFields.ContainsKey("LINKID"))
                            if (user.MergeFields["LINKID"] != "")
                                continue;
                        StringBuilder sb1 = new StringBuilder();

                        sb1.Append(AppHelper.RandomUniqueMailChimp(6));


                        Debug.WriteLine("processing: " + user.EmailAddress + iterator + " of " + members.Count());
                        var generateIdMailChimp = "";
                        WebUser ourUser = _membershipService.GetUserByEmail(user.EmailAddress);
                        if (ourUser != null)
                        {
                            generateIdMailChimp = ourUser.idMailChimp;
                        }
                        ...
                        var memMergeFields = new Dictionary<string, object>();
                        memMergeFields.Add("LINKID", sb1.ToString());
                        ...
                        try
                        {
                            ...
                            if (addMember.Status == Status.Pending)
                            {
                                sb.Append("Status=Pending on: " + user.EmailAddress.ToLower());
                                var a = addMember.Status;
                            }
                        ...
                        {
                            _logger.FatalException("MigrateMailChimpUser: " + user.EmailAddress, e);
                        }
                        var text = "";
                        foreach (var field in memMergeFields)
                        {
            ...
            string intTrainers, string intDes, string intWebinars, Member newMember, List mcList)
        {
            ...
            if (user.Keep_Me_Informed_About_.Contains("ebinars.com"))
            {
