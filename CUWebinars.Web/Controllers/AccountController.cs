...
namespace CUWebinars.Web.Controllers
{
    ...
    [System.Web.Mvc.HttpPost]
        ...
        public JsonResult CheckEmail(string email, bool disregardInstitutionDomain)
        {
            ...
            var resultObject = new Dictionary<string, string>();
            // adds logic to update order with provided email
            // this entry point  is only hit via the prompt by a use case
            // where anon attmpts to CreateUserAccount

            //So should always be the case that CurrentUser has email
            // generated by SessionStart.
            
            // But how to trap via Cart (has order) vs CreateUser (no order)
            // ...probably no need for 2 contingencies. Just issue an UPDATE against
            // any order with SessionStart email so as to reflect the incoming email.
