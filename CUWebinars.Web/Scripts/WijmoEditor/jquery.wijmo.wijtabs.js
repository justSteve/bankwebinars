...
(function (wijmo) {
    ...
    */
    (function (_tabs) {
        ...
        var tabId = 0, listId = 0, effects = $.effects ? $.effects : $, effectsSave = effects.save, effectsRestore = effects.restore, effectsCreateWrapper = effects.createWrapper, effectsRemoveWrapper = effects.removeWrapper, wrapCollapsedCss = "scrollWrap-collapsed", getNextTabId = function () {
            return ++tabId;
        ...
        var wijtabs = (function (_super) {
            ...
            wijtabs.prototype._create = function () {
                ...
                this.originalStyle = this.element.attr("style");
            ...
            */
            ...
            wijtabs.prototype._tabify = function (init) {
                this.list = this.scrollWrap ? this.scrollWrap.find(".wijmo-wijsuperpanel-statecontainer .wijmo-wijsuperpanel-templateouterwrapper ol, ul") : this.element.children('ol,ul').eq(0);
                this.lis = $('li:has(a)', this.list);
                this.originalListStyle = this.list.attr("style");
                ...
                if (init) {
                    ...
                    this.panels.css("display", "none").attr('aria-hidden', true);
                    this.lis.removeClass(o.wijCSS.tabsActive).removeClass(o.wijCSS.stateActive).attr('aria-selected', false);

                    // check for length avoids error when initializing empty list
                    if (o.selected >= 0 && this.anchors.length) {
                        this.panels.eq(o.selected).css("display", "block").attr('aria-hidden', false);
                ...
                if (!self._isDisabled() && o.event !== 'mouseover') {
                ...
                showTab = ((o.showOption.blind || o.showOption.fade) && o.showOption.duration > 0) ? function (clicked, $show) {
                    ...
                    self._showContent();
                    $show.css("display", "block").attr('aria-hidden', false);
                    ...
                    if (tabsAlign === 'Top' || tabsAlign === 'Bottom') {
                        ...
                        $show.hide().css("display", "block").attr('aria-hidden', false).animate(props, o.showOption.duration || 'normal', function () {
                        ...
                        self._showContent();
                        self._scrollWrapCollapsed(false);
                ...
                hideTab = ((o.hideOption.blind || o.hideOption.fade) && o.hideOption.duration > 0) ? function (clicked, $hide) {
                    if (tabsAlign === 'Top' || tabsAlign === 'Bottom') {
                        ...
                        $hide.animate(props, o.hideOption.duration || 'normal', function () {
                            ...
                            $hide.css("display", "none").attr('aria-hidden', true);
                            self.element.dequeue("tabs");
                        });
                    } else {
                        self._saveLayout();
                        self._scrollWrapCollapsed(true);
                    ...
                    // commnet: when tab is sortable, the jquery ui sortable attach mousedown event to tab, the
                    // event prevent default moousedown event, then it make the activateElement caret flicker
                    // on tab header, this only happend on IE11.
                    if (o.sortable && $.browser.msie && $.browser.version && parseFloat($.browser.version) >= 11 && $.contains($hide[0], document.activeElement) && $(document.activeElement).is(":hidden")) {
                ...
                if (!self._isDisabled()) {
                    this.anchors.bind(o.event + '.tabs', function () {
                        var el = this, $li = $(this).closest('li'), $hide = self.panels.filter(function (index) {
            ...
            wijtabs.prototype._scrollWrapCollapsed = function (isWrapCollapsed) {
                var self = this, o = self.options, align = self._getAlignment(true), vert = align === 'Left' || align === 'Right';
                if (self.scrollWrap && o.collapsible && o.scrollable && vert) {
                    self.scrollWrap.toggleClass(wrapCollapsedCss, isWrapCollapsed);
                }
            };
            ...
            wijtabs.prototype._createSortable = function (tabsAlign) {
                ...
                self.list.sortable({
                    activate: function (e, ui) {
                        // Fixing bug 68083 in FireFox:
                        // Jquery-ui sortable has prevented the defult action of "mousedown",
                        // then iframe in wijeditor cannot trigger blur event right after mousedown in the sortable element.
                        // To prevent the wrong stroage of wijeditor, call "blur" manually when sorting begin.
                        if ($.browser.mozilla) {
                            var activeEle = $(document.activeElement), href, panel;
                            if (activeEle.is("iframe")) {
                                href = ui.item.find("a").attr("href"), panel = self.panels.filter(href);
                                if (activeEle.parents().index(panel) > -1) {
                                    activeEle.blur();
                                }
                            }
                        }
                    ...
                    update: function (e, ui) {
                        ...
                        var o = self.options, href = ui.item.find("a").attr("href"), panel = self.panels.filter(href), panelIndex = self.panels.index(panel), newIndex = self.lis.index(ui.item), panelsContainer = tabsAlign === "Left" || tabsAlign === "Right" ? self.element.children("." + o.wijCSS.wijtabsContent) : self.element;
                        if (panelIndex > newIndex) {
                            $(self.panels[newIndex]).before(panel);
                        } else {
                            $(self.panels[newIndex]).after(panel);
                        }
                        self.panels = panelsContainer.children("." + o.wijCSS.tabsPanel);
                    },
            ...
            wijtabs.prototype._blindPanel = function (panel, mode) {
                var self = this, o = self.options, content = panel.parent('.' + o.wijCSS.wijtabsContent), props = ['position', 'top', 'left', 'width'], blindOption = mode === 'show' ? o.showOption : o.hideOption, wrapper, a, listWidth;
                if (!content.length) {
                    return;
                }
                ...
                if (effectsCreateWrapper) {
                    wrapper = effectsCreateWrapper(panel).css({ overflow: 'hidden' });
                } else {
                    wrapper = $('<div></div>');
                }
                if (mode === 'show') {
            ...
            wijtabs.prototype._showContent = function () {
                ...
                if (content.length) {
                    this._restoreLayout();
                    content.css("display", "block").attr('aria-hidden', false);
                }
            };
            wijtabs.prototype._saveLayout = function () {
                var self = this, wijCSS = self.options.wijCSS, props = ['width', 'height', 'overflow'], $hide = self.panels.filter(':visible'), content = self.element.find('.' + wijCSS.wijtabsContent);

                //$.effects.save(self.element, props);
                //$.effects.save(self.list, props);
                //$.effects.save(self.element.find('.' + wijCSS.wijtabsContent), props);
                if (effectsSave) {
                    effectsSave(self.element, props);
                    if (self.list.length) {
                        effectsSave(self.list, props);
                    }
                    if (content.length) {
                        effectsSave(content, props);
                    }
                }
                if (self.list.length) {
                    self.list.width(self.list.width());
                ...
                self.element.data('panel.width', $hide.width());
                self.element.data('panel.outerWidth', $hide.outerWidth(true));
            };

            wijtabs.prototype._restoreLayout = function () {
                var self = this, wijCSS = self.options.wijCSS, props = ['width', 'height', 'overflow'], content = self.element.find('.' + wijCSS.wijtabsContent);
                ...
                if (effectsRestore) {
                    effectsRestore(self.element, props);
                    if (self.list.length) {
                        effectsRestore(self.list, props);
                    }
                    if (content.length) {
                        effectsRestore(content, props);
                    }
                }
            };
            ...
            wijtabs.prototype._resetStyle = function ($el) {
                $el.css({ display: '' });
                if (!$.support.opacity) {
            ...
            wijtabs.prototype._initScroller = function () {
                var _this = this;
                var horz = $.inArray(this._getAlignment(true), ['Top', 'Bottom']) !== -1, width = 0, height;
                ...
                this._removeScroller();
                this.list.css("width", "");
                if (!!this.options.scrollable && horz && this.element.innerWidth() < width) {
                ...
                } else if (!!this.options.scrollable && !horz && this.element.height() < height) {
                    if (this.scrollWrap === undefined) {
                        this.list.wrap("<div class='scrollWrap ui-helper-clearfix'></div>");
                        this.scrollWrap = this.list.parent();
                    ...
                    this.scrollWrap.setOutHeight(this.element.height());
            ...
            wijtabs.prototype._getURLParameters = function (url) {
                var params = {}, parametersString, parameters;
                if (url.indexOf('?') > -1) {
            ...
            wijtabs.prototype._cookie = function (index, cookie) {
                ...
                return $.cookie.apply(null, [c].concat($.makeArray(arguments)));
            };
            ...
            wijtabs.prototype._parsePxToNumber = function (px) {
            ...
            */
            wijtabs.prototype.add = function (url, label, index) {
                ...
                $panel.addClass(o.wijCSS.tabsPanel).addClass(o.wijCSS.content).addClass(o.wijCSS["corner" + panelCorner]).css("display", "none").attr('aria-hidden', true);
        ...
        wijtabs.prototype.options = $.extend(true, {}, wijmo.wijmoWidget.prototype.options, new wijtabs_options());
